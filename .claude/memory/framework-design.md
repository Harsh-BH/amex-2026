...
**UPDATE — v5 category-margin (0.733, NEW BEST):** weighting spend by per-category net margin (airline/lodging net-NEGATIVE from 5x reward cost; dining/other/ent positive — cited rates) scored **0.733, +0.119 over v3**. Lesson: since spend dominates the magnitude ranking, reshaping the spend signal is the highest-leverage lever (a percentile-world test had wrongly called category-weighting low-impact). Trajectory **0.465→0.614→0.733**; ~0.167 below leaders (0.90). Leading engine: `dollar_profit_catmargin`. **v6 (+f4 contingent) HURT (0.675) → f4 stays excluded. v7 cohort-fix WON (0.768, new best):** key finding — `f5` is uncorrelated with real spend (Spearman 0.009) and with profit (−0.038) (A14), so the ~23% no-breakdown cohort was ranked by NOISE; v7 ranks them by `f4` (best proxy, 0.28) where present, else demotes to interest/cost-only → no-breakdown 18%→10.5% of top-20%. Leading engine now `dollar_profit_catmargin_v7`. **Trajectory 0.465→0.614→0.733→0.768**; ~0.132 below leaders. Next: recalibrate per-category margins on the v7 base (dining/travel earn-multiplier hypothesis — high-earn categories may be net-negative too).
