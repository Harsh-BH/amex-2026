...
**External project/codebase scan (GitHub + Kaggle, 2026-06-29) — nothing net-new.** Reviewed franklinanalytics (profit = fee + spend×reward − unpaid; archetypes), debbyrofikomalik (risk/revenue BI dashboard), allmeidaapedro (LightGBM churn→CLV), Kaggle CLV notebooks + peachji dataset, and MCDM libs (pymcdm, topsis-python). Verdict: all either (a) confirm our revenue−cost−risk structure with cruder/economically-weaker formulas (treating rewards as +profit, including the constant fee), (b) are supervised or forward-CLV — the wrong framing (we score a realized snapshot, no label), or (c) MCDM/TOPSIS, already evaluated round-2 and deliberately passed for interpretability + no-dependency. **~4 external passes now converge → external practice validates the framework; stop re-scanning, the bottleneck is the leaderboard.**
